...
def find_smallest_missing_positive(arr):
    ...
    while low <= high:
        ...
        # If arr[mid] == mid + 1, then all elements from 0 to mid are in correct positions
        # The missing element must be in the right half
        if arr[mid] == mid + 1:
            low = mid + 1
        else:
            # There's a gap in the left half (including mid)
            high = mid - 1
    
    # low is now the index where the first missing positive should be
    return low + 1
