...
def prim_mst_heap(graph, start=0):
    ...
    for v in range(V):
        if parent[v] != -1:
            mst_edges.append((parent[v], v, graph[v][parent[v]] if graph[v][parent[v]] else graph[parent[v]][v]))
    return mst_edges
...
if __name__ == "__main__":
    ...
    start = 0
    print(prim_mst_heap(graph, start))  # Expected: [(0,1,2),(1,2,3),(1,4,5),(0,3,6)]
