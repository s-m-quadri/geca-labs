...
# Output: 1
 ...
 def smallest_missing_positive(arr):
    ...
    # If the first element is greater than 1, then 1 is missing
    if arr[0] != 1:
        return 1
    ...
    while low <= high:
        ...
        # Check if number at mid is where it should be
        if arr[mid] == mid + 1:
    ...
    # When loop ends, low points to first index where arr[i] != i + 1
    return low + 1
